...
int static_file_len(int size) { // if you pass in 0, you get the number.
    static int count = 1;
    if (size != 0) {
        count = size;
    }
...
char* parse_line(char *line, int* trail_length, int col_len) {
    ...
    char *return_string = calloc(2048,1);
    char *parsed_tok;
...
int static_var_size(int size) { // if you pass in 0, you get the number.
    static int count = 0;
    if (size) {
        count = size;
    }
